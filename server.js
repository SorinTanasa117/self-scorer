...
// Update a positive for the logged-in user
app.put('/api/positives/:id', authenticateToken, (req, res) => {
  const { id } = req.params;
  const { name, date, score, baseScore, count } = req.body;
  db.get('SELECT id FROM users WHERE username = ?', [req.user.username], (err, userRow) => {
    if (err) {
      return res.status(500).json({ message: 'Internal server error' });
    }
    db.get('SELECT * FROM positives WHERE id = ? AND userId = ?', [id, userRow.id], (err, positive) => {
      if (err) {
        return res.status(500).json({ message: 'Internal server error' });
      }
      if (!positive) {
        return res.status(404).json({ message: 'Positive not found' });
      }
      db.run('UPDATE positives SET name = ?, date = ?, score = ?, baseScore = ?, count = ? WHERE id = ?',
        [name, date, score, baseScore, count, id], function(err) {
        if (err) {
          return res.status(500).json({ message: 'Internal server error' });
        }
        res.json({ message: 'Positive updated successfully' });
      });
    });
  });
});

// Delete a positive for the logged-in user
app.delete('/api/positives/:id', authenticateToken, (req, res) => {
  const { id } = req.params;
  db.get('SELECT id FROM users WHERE username = ?', [req.user.username], (err, userRow) => {
    if (err) {
      return res.status(500).json({ message: 'Internal server error' });
    ...
    db.get('SELECT * FROM positives WHERE id = ? AND userId = ?', [id, userRow.id], (err, positive) => {
      ...
      db.run('DELETE FROM positives WHERE id = ?', [id], function(err) {
        if (err) {
          return res.status(500).json({ message: 'Internal server error' });
        }
        res.json({ message: 'Positive deleted successfully' });
      });
    });
  });
});
...
// Update a custom template for the logged-in user
app.put('/api/templates/:id', authenticateToken, (req, res) => {
  ...
  db.get('SELECT id FROM users WHERE username = ?', [req.user.username], (err, userRow) => {
    ...
    db.get('SELECT * FROM custom_templates WHERE id = ? AND userId = ?', [id, userRow.id], (err, template) => {
      if (err) {
        return res.status(500).json({ message: 'Internal server error' });
      }
      if (!template) {
      ...
      db.run('UPDATE custom_templates SET name = ?, score = ? WHERE id = ?',
        [name, score, id], function(err) {
        if (err) {
          return res.status(500).json({ message: 'Internal server error' });
        }
...
// Delete a custom template for the logged-in user
app.delete('/api/templates/:id', authenticateToken, (req, res) => {
  ...
  db.get('SELECT id FROM users WHERE username = ?', [req.user.username], (err, userRow) => {
    ...
    db.get('SELECT * FROM custom_templates WHERE id = ? AND userId = ?', [id, userRow.id], (err, template) => {
      ...
      db.run('DELETE FROM custom_templates WHERE id = ?', [id], function(err) {
        if (err) {
          return res.status(500).json({ message: 'Internal server error' });
        }
